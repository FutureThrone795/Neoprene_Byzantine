...
use crate::byz_node_sorted_vec::ByzNodeCoefficientAddVec;
use crate::byz_node_sorted_vec::ByzNodePowerMulVec;
...
impl PartialEq for ByzNode {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (ByzNode::Rational { rational }, ByzNode::Rational { rational: rational_other }) => {
                return *rational == *rational_other;
            },
...
impl Ord for ByzNode {
    fn cmp(&self, other: &ByzNode) -> Ordering {
        match (self, other) {
            (ByzNode::Rational { rational }, ByzNode::Rational { rational: rational_other }) => {
                return rational.cmp(rational_other);
            },
...
impl ByzNode {
    /// Returns None if there is no simplification to be done
    pub fn simplified(node: Rc<ByzNode>) -> Option<ByzNode> {
        match node.as_ref() {
